...
export default function DayTradingApp() {
  ...
  const generateInstitutionalAnalysis = () => {
    ...
    // Generate scores based on strike price and DTE (simulating technical analysis)
      ...
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'user',
              content: `Evaluate this ${optionType.toUpperCase()} options trade from an institutional perspective. Be direct and professional.

${ticker} ${optionType.toUpperCase()} @ $${strikePrice} | ${daysToExpiry} DTE

Thesis: ${thesis}

Respond with a 2-sentence professional assessment of edge, risk management, and position sizing considerations.`
            }
          ]
        })
      });
      ...
      const claudeInsight = data.content?.[0]?.text || '';

      const result = generateInstitutionalAnalysis();
      result.claudeInsight = claudeInsight.trim();

      setAnalysisResult(result);
    } catch (err) {
      const result = generateInstitutionalAnalysis();
      result.claudeInsight = 'Analysis complete. Review scores and Greeks carefully.';
      setAnalysisResult(result);
    } finally {
